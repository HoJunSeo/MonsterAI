    ...
    BaseBoard MakeBoard(eBoardType boardType)
    {
        ...
        switch (boardType)
        {
            ...
            case eBoardType.BOARD_HP:
                {
                    GameObject hpBoard = NGUITools.AddChild(BoardUI, DicPrefab[eBoardType.BOARD_HP]);   // 부모설정 + 딕프리팹 설정
            ...
            case eBoardType.BOARD_DAMAGE:
                {
                    GameObject damagerBoard = NGUITools.AddChild(BoardUI, DicPrefab[eBoardType.BOARD_DAMAGE]);
                    boardData = damagerBoard.GetComponent<DamageBoard>();
                }
                break;
            default:
                break;
