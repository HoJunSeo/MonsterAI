...
public class SkillManager : MonoSingleton<SkillManager>
{
    ...
    public void RunSkill(BaseObject baseObject, string strSkillTemplateKey)
    {
        SkillTemplate template = GetSkillTemplate(strSkillTemplateKey);

        if(template == null)
        {
            Debug.Log(strSkillTemplateKey + " Skill Template 키를 찾을 수 없습니다.");
            return;
        }

        BaseSkill runSkill = CreateSkill(baseObject , template);
        RunSkill(baseObject, runSkill);
    }
